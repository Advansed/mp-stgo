...
import styles from './ActCCForm.module.css';
...
interface ActCCFormProps {
  onBack: () => void;
  invoice_id?: string;
}

export const ActCCForm: React.FC<ActCCFormProps> = ({ onBack, invoice_id }) => {
  const toast = useToast();
  const user = useLoginStore(state => state.user);
  const [isEditing, setIsEditing] = useState(false);
  const [previewData, setPreviewData] = useState<PageData | null>(null);
  const { actData, isLoading, loadData, saveData, get_pdf } = useActCC();
...
<div class="container">
  ...
  const getFormData = (act?: MeterReplacementData | null): PageData => [
    {
  ...
  const handleSave = (data: PageData) => {
    const formData: MeterReplacementData = {
