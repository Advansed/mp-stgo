...
import { t_actmr } from '../../../constants/templates';
import { useActMR } from './useActMR';

interface ActMRViewProps {
  act: ActData | null;
  onBack?: () => void;
  onEdit: () => void;
}

export const ActMRView: React.FC<ActMRViewProps> = ({ act, onBack, onEdit }) => {
  const toast = useToast();
  const [modal, setModal] = useState<string | undefined>(undefined);
  const { isLoading, get_pdf } = useActMR();

  const handlePrint = async () => {
    if (!act || act.type !== 'actmr') return;

    const res = await get_pdf(t_actmr, act.details, act.act_number, act.act_date);
    if (res.success) {
      setModal(res.data);
    ...
    <>
      ...
      <div className={styles.batteryActForm}>
        ...
        <div className={styles.formHeaderLeft}>
          ...
          <h2 className={styles.formTitle}><b>Акт замены газового счетчика</b></h2>
        </div>
