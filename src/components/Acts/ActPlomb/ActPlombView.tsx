// Acts/ActPlomb/ActPlombView.tsx
...
import { t_actplomb } from '../../../constants/templates';
import { useActPlomb } from './useActPlomb';

interface ActPlombViewProps {
  act: ActData | null;
  onBack?: () => void;
  onEdit: () => void;
}

export const ActPlombView: React.FC<ActPlombViewProps> = ({ act, onBack, onEdit }) => {
  const toast = useToast();
  const [modal, setModal] = useState<string | undefined>(undefined);
  const { isLoading, get_pdf } = useActPlomb();

  const handlePrint = async () => {
    if (!act || act.type !== 'actplomb') return;

    const res = await get_pdf(t_actplomb, act.details, act.act_number, act.act_date);
    if (res.success) {
      setModal(res.data);
    ...
    return [
      ...
      },
      {
        ...
        data: [
          ...
          { label: 'Номер счетчика', value: actData?.meter_number || '—' },
          { label: 'Номер пломбы', value: actData?.seal_number || '—' },
          { label: 'Примечание', value: actData?.note || '—' },
        ],
      },
      {
